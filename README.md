...
[How to run Gotcha](https://github.com/landau-lab/Gotcha/blob/multi_os_compatible/GoTChA_pipeline.Rmd)
...
1 - FastqSplit: this function splits the fastq files into chunks for parallel computing by the BatchMutationCalling function.

2 - FastqFiltering: reads in the fastqs generated by gotcha, and filters them based on pre-determined base quality scores.
...
4 - BatchMutationCalling: this function applies the MutationCalling function to the fastq files split by FastqSplit. It calls MutationCalling in parallel, using one core for each subset of fastq files. Using this function, processing a GoT-ChA genotyping library of 40 million reads takes ~ 7h. As output, it generates a data frame within each folder of the fastq split files, that can be merged using the following function.
